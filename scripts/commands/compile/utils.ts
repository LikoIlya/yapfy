...
export const compile = async (options) => {
  ...
  const outputDirectory =
    options.format === "json"
      ? config.outputDirectory
      : config.contractsDirectory + "/../" + "compiled";
  let additional_path = (options.contract as string).substring(0, options.contract.lastIndexOf('/'));
  if (!fs.existsSync(outputDirectory+"/"+additional_path)) {
    console.log(
      `Creating output directory "${outputDirectory+"/"+additional_path}" since it was not present.`
    );
    fs.mkdirSync(outputDirectory+"/"+additional_path, { recursive: true });
  }
...
export const compileExpression = (
  ...
  const init_file = `$PWD/contracts/${init}.ligo`;
  try {
    const params = `'${expression}' --michelson-format json --init-file ${init_file} --protocol hangzhou`;
    const command = `${ligo} ${ligo_command} ${config.preferredLigoFlavor} ${params}`;
    const michelson = execSync(command, { maxBuffer: 1024 * 1000 }).toString();
    if (output) {
      if (!fs.existsSync(`$PWD/${config.outputDirectory}/compiled`)) {
        fs.mkdirSync(`$PWD/${config.outputDirectory}/compiled`, {
          recursive: true,
        });
      }
      const save_path = `$PWD/${config.outputDirectory}/compiled/${output}.json`;
      fs.writeFileSync(save_path, JSON.parse(michelson).bytes);
      console.log(`Saved to ${save_path}`);
    }
    else console.log(michelson);
  } catch (e) {
    console.error(e);
  }
};